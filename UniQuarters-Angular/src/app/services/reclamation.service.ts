...
export class ReclamationService {
  ...
  addReclamation(e: Reclamation): Observable<HttpResponse<any>> {
    console.log('reclamation to add', e);

    return this.http.post(url, e, { observe: 'response' }).pipe(
      retry(3),
  ...
  updateReclamation(e: Reclamation): Observable<HttpResponse<any>> {
    return this.http.put(url, e, { observe: 'response' }).pipe(
      retry(3),
      catchError(this.handleError),
      tap(() => {
        this._refresh$.next();
